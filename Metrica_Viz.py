...
def plot_shots( shots, figax=None, field_dimen = (106.0,68), indicators = ['Marker','Arrow'], goal_color='b',miss_color='r', annotation_colour='white', goal_style = 'o', miss_style = 'x', goal_alpha = 0.5,miss_alpha = 0.3, annotate=False):
    """ plot_events( events )
    ...
    for i,row in shots.iterrows():
        if 'Marker' in indicators:
            if row['Subtype'].find('-GOAL') != -1:
                ax.plot(  row['Start X'], row['Start Y'], goal_color+goal_style, alpha=goal_alpha )
            else: 
                ax.plot(  row['Start X'], row['Start Y'], miss_color+miss_style, alpha=miss_alpha)
        if 'Arrow' in indicators:
            if row['Subtype'].find('-GOAL') != -1:
                ax.annotate("", xy=row[['End X','End Y']], xytext=row[['Start X','Start Y']], arrowprops=dict(arrowstyle="->",color=goal_color, alpha=goal_alpha),annotation_clip=False)
            else: 
                ax.annotate("", xy=row[['End X','End Y']], xytext=row[['Start X','Start Y']], arrowprops=dict(arrowstyle="->",color=miss_color, alpha=miss_alpha),annotation_clip=False)
            
        if annotate:
            textstring = row['Type'] + ': ' + row['From']
            ax.text( row['Start X'], row['Start Y'], textstring, fontsize=10, color=annotation_colour)
    return fig,ax
