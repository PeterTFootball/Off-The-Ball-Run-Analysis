...
Module for measuring player acceleration, smoothed using a Savitzky-Golay filter, with Metrica tracking data.
...
def calc_player_acceleration(team, smoothing=True, filter_='Savitzky-Golay', window=7, polyorder=1, maxspeed = 12, maxacceleration = 3.09):
    """ calc_player_acceleration( tracking_data )
    
    Calculate player acceleration in x & y direciton, and total player speed at each timestamp of the tracking data
    ...
    -----------
        ...
        filter: type of filter to use when smoothing the acceleration. Default is Savitzky-Golay, which fits a polynomial of order 'polyorder' to the data within each window
    ...
    for player in player_ids: # cycle through players individually
        ...
        if maxspeed>0:
            # remove unsmoothed data points that exceed the maximum speed (these are most likely position errors)
            raw_acceleration = np.sqrt( ax**2 + ay**2 )
            ax[ raw_acceleration>maxacceleration ] = np.nan
            ay[ raw_acceleration>maxacceleration ] = np.nan
